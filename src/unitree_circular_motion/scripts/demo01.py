...
class CircleMover(Node):
    def __init__(self):
        ...
        self.declare_parameter('angular_velocity', 5.0) # rad/s
...
#         self.duration = 0
#         self.task = 1  # Diference task
#         self.executions = 0  # 0
#         self.target_executions = 2  # 2

#     def move_in_circle(self):
#         twist = Twist()

#         if self.task == 1:  # First task: Move only one circle
#             self.duration = 30  
#             if time.time() - self.start_time <= self.duration:
#                 twist.linear.x = 0.2  # Line speed (unit: m/s)
#                 twist.angular.z = 0.2  # Angular velocity (unit: rad/s)
#             else:
#                 twist.linear.x = 0.0
#                 twist.angular.z = 0.0
#                 self.get_logger().info("Completed Task 1: Moved one circle")
#                 self.destroy_node()
        
#         elif self.task == 2:  # Second task: Move twice and stop
...
#                 twist.linear.x = 0.0
#                 twist.angular.z = 0.0
#                 self.get_logger().info("Completed Task 2: Moved two circles")
#                 self.destroy_node()
...
# def main(args=None):
#     rclpy.init(args=args)
#     task = int(input("Enter Task Number (1 for one circle, 2 for two circles): "))
#     node = CircleMovement()
#     node.task = task
#     rclpy.spin(node)
#     node.destroy_node()
