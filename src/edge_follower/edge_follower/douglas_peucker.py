...
class EdgeFollowerNode(Node):
    def __init__(self):
        ...
        # Edge smoothing parameters - updated
        self.SMOOTHING_ANGLE_THRESHOLD = 0.3  # Increased for better corner detection
        self.MIN_EDGE_LENGTH = 0.05  # Minimum edge length to consider
        self.DOUGLAS_PEUCKER_EPSILON = 0.05  # For line simplification
        ...
        self.STUCK_DISTANCE_THRESHOLD = 0.10  # 20 cm
        self.STUCK_TIME_THRESHOLD = 3.0  # 2 seconds
        
        # Path for storing position data
        self.data_file = "/usr/app/comp0244_ws/comp0244-go2/src/edge_follower/edge_follower/parameters.json"
    ...
    def smooth_edges(self, points):
        """Improved edge smoothing using Douglas-Peucker algorithm and angle-based filtering"""
        if len(points) < 3:
            return points
        
        # First apply Douglas-Peucker algorithm to simplify the polyline
        simplified_points = self.douglas_peucker(points, self.DOUGLAS_PEUCKER_EPSILON)
        
        # Then apply angle-based filtering to preserve important corners
        smoothed = [simplified_points[0]]  # Add first point
        
        for i in range(1, len(simplified_points)-1):
            # Vectors between previous and next points
            prev_vector = simplified_points[i] - simplified_points[i-1]
            next_vector = simplified_points[i+1] - simplified_points[i]
            ...
            if angle > self.SMOOTHING_ANGLE_THRESHOLD:
                smoothed.append(simplified_points[i])
        
        smoothed.append(simplified_points[-1])  # Add last point
        return smoothed
    ...
    def douglas_peucker(self, points, epsilon):
        ...
        if line_len < 1e-6:  # If start and end points are very close
            return [points[0]]
        ...
        for i in range(1, end):
            ...
            proj_point = points[0] + line_unit_vec * proj_len
            distance = np.linalg.norm(points[i] - proj_point)
        ...
        if dmax > epsilon:
            ...
            # All points are within epsilon distance, so just return endpoints
    ...
    def find_next_waypoint(self):
        ...
        if closest_edge is None:
            self.get_logger().info('No edge found, activating stuck behavior')
            return None
            ...
            while increment_left > 0 and current_index >= 0:
                ...
                if increment_left <= remaining_distance:
                    ...
                    if current_index < 0:
                        ...
                        edge_vector = end - start
                        edge_direction = edge_vector / np.linalg.norm(edge_vector)
                        current_point = start - edge_direction * 0.70  # Move 25cm back
                        break
                    else:
    ...
    def stuck_forced(self):
        ...
        waypoint_msg.theta = self.current_orientation + math.radians(45)  # 20 -> 30 derece sağa
        # waypoint_msg.theta = self.current_orientation - math.radians(22)  # 20 -> 30 derece sağa
        self.waypoint_pub.publish(waypoint_msg)
        return
    ...
    def stuck_rotation(self):
        ...
        waypoint_msg.x = new_x
        waypoint_msg.y = new_y
        waypoint_msg.theta = self.current_orientation + math.radians(90)
        self.waypoint_pub.publish(waypoint_msg)
