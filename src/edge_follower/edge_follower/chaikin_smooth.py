...
class EdgeFollowerNode(Node):
    ...
    def find_next_waypoint(self):
        ...
        for i, edge in enumerate(self.current_edges):
            ...
            if np.linalg.norm(start_point - robot_position_robot) > 2.0:
            ...
            # For left-side following:
            # We need to check if the robot is on the left side of the edge
            # Cross product between edge direction and (point_on_edge to robot) tells us the side
            to_robot = robot_pos - point_on_edge
            
            # For left-side following, we want positive cross product (robot is on the left of the edge)
                ...
                if increment_left <= remaining_distance:
                    ...
                    if current_index >= len(self.current_edges):
                        ...
                        if current_index >= len(self.current_edges) - 1:  # Son edge'deyiz
                            ...
                            current_point = end + edge_direction * 0.25  # 35cm -> 25cm
                            break
            ...
            while increment_left > 0 and current_index >= 0:
                ...
                if increment_left <= remaining_distance:
                    ...
                    if current_index < 0:
                        ...
                        edge_vector = end - start
                        edge_direction = edge_vector / np.linalg.norm(edge_vector)
                        current_point = start - edge_direction * 0.50  # Move 25cm back
                        break
                    else:
    ...
    def stuck_forced(self):
        ...
        new_x = self.current_x + forward_vector[0] * 0.35  # 35cm -> 25cm
        new_y = self.current_y + forward_vector[1] * 0.35
        
        waypoint_msg = Pose2D()
        waypoint_msg.x = new_x
        waypoint_msg.y = new_y
        waypoint_msg.theta = self.current_orientation + math.radians(30)  # 20 -> 30 derece sağa
...
def main(args=None):
    ...
    # Path to JSON file
    data_file = "/usr/app/comp0244_ws/comp0244-go2/src/edge_follower/edge_follower/parameters.json"
    ...
    try:
        ...
        try:
            ...
            json_data = {"last_position": None, "position_check_time": None}
