...
class EdgeFollowerNode(Node):
    def __init__(self):
        ...
        self.STUCK_DISTANCE = 0.05  # 5cm movement threshold
        self.FORCE_MOVE_DISTANCE = 0.5  # 30cm forward movement when stuck
        ...
        if moving_forward:
            while increment_left > 0 and current_index < len(self.current_edges):
                ...
                if increment_left <= remaining_distance:
                    ...
                    if current_index >= len(self.current_edges):
                        ...
                        edge_start, edge_end = self.current_edges[current_index]
                        edge_direction = (edge_end - edge_start) / np.linalg.norm(edge_end - edge_start)
                        # current_point = self.current_edges[current_index][1]
                        current_point = edge_end + edge_direction * 0.8 # 80 cm forward
                        break
            ...
            while increment_left > 0 and current_index >= 0:
                ...
                if increment_left <= remaining_distance:
                    ...
                    # Move to previous edge (for counterclockwise movement)
                    increment_left -= remaining_distance
                    current_index -= 1  # Move backwards through edges
                    if current_index < 0:
                        # If we reach the start, stay at the first point and move extra 80cm
                        current_index = 0
                        edge_start, edge_end = self.current_edges[current_index]
                        edge_direction = (edge_end - edge_start) / np.linalg.norm(edge_end - edge_start)
                        # Move 80cm in the edge direction from the start point
                        current_point = edge_start - edge_direction * 0.8  # 80 cm backward
                        break
        ...
        if self.last_position is not None:
            ...
            if distance_moved < self.STUCK_DISTANCE:
                if self.stuck_start_time is None:
                    ...
                    self.get_logger().info('Robot might be stuck, starting timer')
                else:
                    stuck_duration = (current_time - self.stuck_start_time).nanoseconds / 1e9
                    if stuck_duration > self.STUCK_THRESHOLD:
                        self.get_logger().warn(f'Robot stuck for {stuck_duration:.1f} seconds, forcing forward movement')
                        
                        # Create forced forward movement waypoint
                        forward_vector = np.array([
                            math.cos(self.current_orientation),
                            math.sin(self.current_orientation)
                        ])
                        forced_point = current_position + forward_vector * self.FORCE_MOVE_DISTANCE
                        
                        # Publish forced waypoint
                        waypoint_msg = Pose2D()
                        waypoint_msg.x = float(forced_point[0])
                        waypoint_msg.y = float(forced_point[1])
